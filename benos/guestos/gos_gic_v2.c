...
static void gicv2_mask_irq(int irq)
{ gic_set_irq(irq, GIC_DIST_ENABLE_CLEAR); }

void gos_gicv2_unmask_irq(int irq)
{
...
void gos_gic_handle_irq(void)
{
	...
	do {
		...
		if (irqnr == GENERIC_TIMER_IRQ || irqnr == V_TIMER_IRQ)
			gos_handle_vtimer_irq();
...
int gos_gic_init(int chip, unsigned long dist_base, unsigned long cpu_base)
{
